/**
 * @created unknown
 * @description 重建观测层级并在确认导出后记录完成状态。
 * @author yunhungo
...
function makeClip(maxChars: number): Clip {
  ...
  function clip(value: unknown): unknown {
    if (Array.isArray(value)) return value.map((item) => clip(item));
...
export async function convertRollout(
  rolloutFile: string,
  options: { config: Config; parentObservation?: LangfuseObservation; flush?: () => Promise<void> },
): Promise<void> {
